...
simple_cross_validation <- function(list_of_minbucket) {
  ...
  #dataframe vac韔 con la estructura indicada
...
list_minbucket <- seq(from=300, to=2000, by=150)
set.seed(1234)
...
#Visualizaci髇 sensibilidad para los distintos minbuckets
size_optimo <- rev(cross_val_simple$minbucket)[which.max(rev(cross_val_simple$sensitivity))]
paste("Tama駉 髉timo encontrado:", size_optimo)
ggplot(data = cross_val_simple, aes(x = minbucket, y = sensitivity)) +
  geom_line() + 
  geom_point() +
  geom_vline(xintercept = size_optimo, color = "red") +
  labs(title = "Sensibilidad vs tama駉 del 醨bol") +
  theme_bw()

arbol450 <- rpart(factor(diabetes)~., data = nhs, 
                  minbucket =450, method = "class", maxsurrogate=0, cp=0, 
                  parms=list(split="gini")) #division gini
rpart.plot(arbol450,extra=1)
#summary(arbol450)

arbol600 <- rpart(factor(diabetes)~., data = nhs, 
                  minbucket =600, method = "class", maxsurrogate=0, cp=0, 
                  parms=list(split="gini")) #division gini
rpart.plot(arbol600,extra=1)
#summary(arbol600)

arbol300 <- rpart(factor(diabetes)~., data = nhs, 
                  minbucket =300, method = "class", maxsurrogate=0, cp=0, 
                  parms=list(split="gini")) #division gini
rpart.plot(arbol300,
...
#-----5. Mejor 醨bol-----
#Para sacar todas las variables m醩 f醕il (QUITAR LA VARIABLE OBJETIVO)
...
set.seed(1234)
cruzadaarbol450<-cruzadaarbolbin(data=nhs, vardep="diabetes", listconti=c(
...
listclass=c(""),grupos=4,sinicio=1234,repe=10,cp=0,minbucket=450)

cruzadaarbol450$modelo="arbol450"

cruzadaarbol600<-cruzadaarbolbin(data=nhs, vardep="diabetes", listconti=c(
  'surveyyr.2009_10','gender.male','age','race1.Black','race1.Hispanic',
  'race1.Mexican','race1.White','race1.Other','education.8th_Grade',
  'education.9_11th_Grade','education.College_Grad','education.High_School',
  'education.NotAsked','education.Some_College','maritalstatus.Married',
  ...
  'samesex.Yes','bpsys','bpdia','marij.HasTried','marij.MarijUser','marij.NA',
  'marij.Never','marij.NotAsked'),
  listclass=c(""),grupos=4,sinicio=1234,repe=10,cp=0,minbucket=600)

cruzadaarbol600$modelo="arbol600"

cruzadaarbol300<-cruzadaarbolbin(data=nhs, vardep="diabetes", listconti=c(
  'surveyyr.2009_10','gender.male','age','race1.Black','race1.Hispanic',
  'race1.Mexican','race1.White','race1.Other','education.8th_Grade',
  'education.9_11th_Grade','education.College_Grad','education.High_School',
  'education.NotAsked','education.Some_College','maritalstatus.Married',
  ...
  'samesex.Yes','bpsys','bpdia','marij.HasTried','marij.MarijUser','marij.NA',
  'marij.Never','marij.NotAsked'),
  listclass=c(""),grupos=4,sinicio=1234,repe=10,cp=0,minbucket=300)

cruzadaarbol300$modelo="arbol300"

unionarbol <- rbind(cruzadaarbol450,cruzadaarbol600,cruzadaarbol300)
...
#Mejor modelo minbucket=300

#-----6. Variables importantes arbol-----
arbol300$variable.importance

imp <- arbol300$variable.importance
...
imp_data %>%
  ...
  coord_flip()+
  labs(title = "Importancia de las variables en el 醨bol ganador")

#Las 5 variables mas importantes son 
#age, totchol, healthgen.Fair, weight y healthgen.Vgood