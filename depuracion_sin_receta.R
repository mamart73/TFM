...
#----2.Importar dataset----
nhanes <- NHANESraw
#Pasamos el nombre de todas las variables a minúscula y borramos NA en la variable objetivo
...
#NA), pero con la diferencia de que race3 tiene una categoría más (Asian), la 
#cual está contemplada en el grupo Other en race1. Al estar recogida únicamente 
#en una de las dos encuestas, nos quedamos solo con race1.

### hhincome -> 
#Se elimina. Es una variable categórica que representa el ingreso bruto anual
#por intervalos. Existe variable análoga numérica (hhincomemid), la cuál tiene 
#como valor elpunto intermedio de los extremos de los intervalos de esta 
#variable. Nos conviene quedarnos con la versión numerica

### bmicatunder20yrs ->
#Se elimina. Es una categorización para menores de 20 años de la variable BMI, 
#la cual se obtiene a partir del peso y la altura.

### bmi_who ->
#Se elimina. Es una categorización de la variable BMI, la cual se obtiene a 
...
#12 o más años. Existe una variable numérica (physactivedays), la cual refleja 
#el número medio de días en una semana en que el participante realiza ejercicio 
#de intensidad moderada o vigorosa. Procederemos a fusionar estas 2 variables, 
#imputando en la numérica como valor 0 (días) a los participantes que hayan
...
#por la forma en que ha sido recogida no es fiable la información que 
...
#no está embarazada, prevalece lo que ella dice a una prueba médica...), 
...
#otra variable que representa la edad en años y no tiene NA, por lo que optamos 
...
#Veamos la correlación de estas variables ya que a priori parece que van a tener
#alta correlación
...
#Se decide eliminar hhincomemid ya que tiene mayor número de NA y se trata de 
...
#datos má excatos sobre el nivel de vida del participante, lo que nos puede
#ayudar a la hora de predecir la variable objetivo.

#nhanes %>% group_by(hhincomemid) %>% summarise(media=mean(poverty, na.rm=TRUE))

### headcirc ->
#Se elimina. Refleja la longitud de bebés menores a 6 meses. Tiene un 100% de NA
...
#participante. Además, hay otra variable (urinevol1) que refleja lo mismo pero
#en otra lectura.

### urineflow2 ->
#Se elimina. Tiene casi 90% de NA y refleja el flujo de orina de un
#participante. Además, hay otra variable (urineflow2) que refleja lo mismo pero
#en otra lectura.
...
#Estas variables reflejan distintas lecturas de la presión arterial sistólica.
#Tiene coeficientes de correlación de Pearson muy cercanos a 1 entre ellas dos a
#dos, por lo que estaríamos dándole 4 veces más importancia a esta variable que 
#al resto si mantenemos las 4 variables. Para no excluir ninguna de las 4 
#lectuas, crearemos una variable nueva que será la media de todas las variables,
#siempre y cuando no tengan valor NA. En este caso, se calculará la media de
#aquellas cuyo valor no sea NA

### bpdiaave, bpdia1, bpdia2, bpdia3 ->
#Estas variables reflejan distintas lecturas de la presión arterial diastólica.
#Tiene coeficientes de correlación de Pearson muy cercanos a 1 entre ellas dos a
#dos, por lo que estaríamos dándole 4 veces más importancia a esta variable que 
#al resto si mantenemos las 4 variables. Para no excluir ninguna de las 4 
#lectuas, crearemos una variable nueva que será la media de todas las variables,
...
#Las eliminamos. Son variables de peso estadístico de las cuales no haremos uso
#en el desarrollo de este trabajo

#Imputamos como NA valores incoherentes en physactive, 
#incluimos info de physactive en physactivedays,
#creamos las variables media de la presiçon arterial diasçoolica y presión 
...
#En este dataset tenemos participantes de hasta 17 años
nhanesyoungraw %>% skim()

#CATEGORICAS

### surveyyr ->
#Refleja el año de la encuesta. No tiene NA
...
#Teniendo en cuenta los porcentajes de observaciones en cada categoría 
#decidimos reagrupar las categorías Rent y Other.
#Tiene menos de 1% de NA. Se imputan

### work ->
#Refleja la relación con respecto al trabajo. Tiene más de un 90% de NA. Esto
#se puede deber a que en EEUU la edad mínima para trabajar es 14 años. 
#Se podrían imputar como NotWorking en menores de 14 años. Veamos como quedaría
#la distribucion
...
#Debido a la poca variabilidad de esta variable y su relación con la variable 
...
### healthgen ->
#Refleja una calificación categórica del estado de salud autopercibido y cuenta 
#con 5 categorías. Tiene 74% de NA. Esta variable ha sido recogida para 
#participantes de 12 o más años, por lo que imputaremos este rango de edad no 
...
#Teniendo en cuenta los porcentajes de observaciones en cada categoría y la
#relación de cada una de estas con la variable objetivo decidimos reagrupar las
#categorias:
#Excellent y Vgood
#Fair, Poor y Good

#Por otro lado, a pesar de que los NA puedan tener algun significado, al tratarse
#de una cantidad tan pequeña los imputamos

### littleinterest ->
#Refleja una calificación categórica del número de días que el participante 
#ha tenido interés en hacer cosas. Tiene un 100% de NA debiendose esto a que
#esta variable ha sido recogida para participantes de 18 o más años, por lo que
#en este dataset la ELIMINAMOS

### depressed ->
#Refleja una calificación categórica del número de días que el participante 
#se ha sentido depresivo. Tiene un 100% de NA debiendose esto a que esta 
#variable ha sido recogida para participantes de 18 o más años, por lo que
#en este dataset la ELIMINAMOS

### sleeptrouble ->
#Se trata de una variable binaria en la que se expone si el participante le ha
#dicho a algún profesional de la salud si ha tenido problemas para dormir. 
#Tiene más de un 90% de NA debiendose esto a que esta variable ha sido recogida 
#para participantes de 16 o más años
...
#Debido a su gran cantidad de NA y poca relación con la variable objetivo, la
#ELIMINAMOS

### tvhrsday ->
#Refleja una calificación categórica del número medio de horas diarias que el 
#participante ha visto la televisión durante los últimos 30 días y cuenta con
#6 categorías. Esta variable ha sido recogida para participantes de 2 o más 
#años en la encuesta 2011-2012, mientras que no ha sido recogida en la encuesta
#2009-2010. Existe una variable numérica (tvhrsdaychild), la cual refleja 
#el número medio de horas al día en los últimos 30 días en las que el 
#participante vio la televisión, recogida para participantes entre 2 y 11 años 
#en la encuesta del año 2009-2010. Haremos una fusión entre ambas variables para
...
#Debido a las cantidades tan pequeñas en cada categoria, decidimos hacer las 
...
### comphrsday ->
#Refleja una calificación categórica del número medio de horas diarias que el 
#participante ha usado el ordenador durante los últimos 30 días y cuenta con
#6 categorías. Esta variable ha sido recogida para participantes de 2 o más 
#años en la encuesta 2011-2012, mientras que no ha sido recogida en la encuesta
#2009-2010. Existe una variable numérica (comphrsdaychild), la cual refleja 
#el número medio de horas al día en los últimos 30 días en las que el 
#participante uso el ordenador, recogida para participantes entre 2 y 11 años 
#en la encuesta del año 2009-2010. Haremos una fusión entre ambas variables para
#recoger la maxima informacion posible y crearemos una categoria nueva NotAsked
#para aquellos participantes que no hayan respondido a esta variable. La nueva 
#variable "fusion" sera categorica. Ademas, habria que considerar los NA reales 
...
#bebido al menos 12 copas en cualquier año. Tiene 100% de NA ya que ha sido 
#recogida para participantes de 18 años o más, por lo que la ELIMINAMOS
...
#participantes de 20 años o más, por lo que la ELIMINAMOS

### smoke100 ->
#Se trata de una variable binaria en la que se expone si el participante ha 
#fumado mas de 100 cigarrillos en toda su vida. Tiene 100% de NA ya que ha sido 
#recogida para participantes de 20 años o más, por lo que la ELIMINAMOS

### marijuana ->
#Se trata de una variable binaria en la que se expone si el participante ha 
#probado la marihuana. Tiene 100% de NA ya que ha sido recogida para 
#participantes de entre 18 y 59 años, por lo que la ELIMINAMOS
...
### harddrugs ->
#Se trata de una variable binaria en la que se expone si el participante ha
#probado cocaína, crack, heroína o metanfetamina. Tiene 100% de NA ya que ha 
#sido recogida para participantes de entre 18 y 69 años, por lo que la ELIMINAMOS

### sexever ->
#Se trata de una variable binaria en la que se expone si el participante ha
#tenido sexo vaginal, anal u oral. Tiene 100% de NA ya que ha sido recogida para
#participantes de entre 18 y 69 años, por lo que la ELIMINAMOS

### samesex ->
#Se trata de una variable binaria en la que se expone si el participante ha
#tenido sexo con alguien de su mismo sexo. Tiene 100% de NA ya que ha sido 
#recogida para participantes de entre 18 y 69 años, por lo que la ELIMINAMOS

### sexorientation ->
#Esta variable refleja la orientación sexual del participante. Tiene 100% de NA
#ya que ha sido recogida para participantes de entre 18 y 59 años, por lo que la
#ELIMINAMOS
...
#años. Se fusionará con la variable height
...
### pulse ->
#Esta variable refleja la frecuencia del pulso del participante. Tiene más de un
#50% de NA
...
#relevantes para imputar más del 50% de los valores que requiere esta variable.
#Por ello, la eliminamos

### testosterone ->
#Esta variable refleja el nivel de testosterona total en ng/dL y ha sido 
#recogida únicamente en la encuesta del año 2011-2012. Tiene más de 75% de NA.
...
#A pesar de que existan diferencias notables entre estadísticos, el hecho de 
...
### daysphyshlthbad ->
#Esta variable refleja el número de días que el participante ha considerado
#que su salud fisica no fue buena en los últimos 30 días. Tiene 60% de NA y ha
#sido recogida para participantes de 12 o más años. Debido a la cantidad de NA
#y la información que representa convertiremos entro variable a categórica
#dándole distintaas categorías. Además, imputaremos el rango de edad no 
...
#Debido a la cantidad de NA de esta variable y su relación con la variable
#objetivo, se elimina

### daysmenthlthbad ->
#Esta variable refleja el número de días que el participante ha considerado
#que su salud mental no fue buena en los últimos 30 días. Tiene 60% de NA y ha
#sido recogida para participantes de 12 o más años. Debido a la cantidad de NA
#y la información que representa convertiremos entro variable a categórica
#dándole distintaas categorías. Además, imputaremos el rango de edad no 
#contemplado en una nueva categoria a la que llamaremos NotAsked
...
#embarazada y ha sido recogida para mujeres de 20 años o mas. Tiene un 100% de
#NA, por lo que la ELIMINAMOS

### nbabies ->
#Esta variable refleja el numero de veces que la participante ha tenido bebés y 
#ha sido recogida para mujeres de 20 años o mas. Tiene un 100% de NA, por lo que
#la ELIMINAMOS

### age1stbaby ->
#Esta variable refleja la edad en la que la partipante ha sido madre por 1ª vez 
#y ha sido recogida para mujeres de 20 años o mas. Tiene un 100% de NA, por lo 
#que la ELIMINAMOS

### sleephrsnight ->
#Esta variable refleja el número de horas de sueño que el participante tiene por
#la noche en días de trabajo o fin de semana. Tiene un 85% de NA y se ha 
#preguntado a participantes de 16 años o mas. Debido a la alta cantidad de NA,
#procedemos a eliminarla

### physactivedays ->
#Esta variable refleja el número medio de días en una semana en que el 
#participante realiza ejercicio de intensidad moderada o vigorosa. Tiene un 
#59% de NA y se ha recogido para participantes de 12 o mas años. 
#Debido a la cantidad de NA y la información que representa 
#convertiremos esta variable a categórica dándole distintaas categorías. 
...
#imputar los menores de 12 años como NotAsked
...
### alcoholday ->
#Esta variable refleja el número medio de bebidas consumidas por el participante
#al día en aquellos días que consume bebidas alcohólicas y ha sido recogida para 
#particpantes de 18 o más años. Tiene un 100% de NA, por lo que la ELIMINAMOS

### alcoholyear ->
#Esta variable refleja el número de días del año pasado en los que el 
#participante consumió bebidas alcohólicas y ha sido recogida para 
#participantes de 18 o más años. Tiene un 100% de NA, por lo que la ELIMINAMOS

### smokeage -> 
#Esta variable refleja la edad en la que el participante comenzó a fumar de 
#manera regular y ha sido recogida para participantes de 20 o más años. Tiene 
#un 100% de NA, por lo que la ELIMINAMOS

### agefirstmarij -> 
#Esta variable refleja la edad en la que el participante probó la marihuana por
#primera vez y ha sido recogida para participantes de entre 18 y 59 años. Tiene 
#un 100% de NA, por lo que la ELIMINAMOS

### ageregmarij ->
#Esta variable refleja la edad a la que el participante comenzó a consumir 
#marihuana de manera regular y ha sido recogida para participantes de entre 18 y
#59 años. Tiene un 100% de NA, por lo que la ELIMINAMOS

### sexage ->
#Esta variable refleja la edad en la que el participante tuvo sexo por primera
#vez y ha sido recogida para participantes de entre 18 y 69 años. Tiene un 100% 
#de NA, por lo que la ELIMINAMOS

### sexnumpartnlife ->
#Esta variable refleja el número de parejas del sexo opuesto con las que el
#participante ha tenido algún tipo de relación sexual en toda su vida y ha sido 
#recogida para participantes de entre 18 y 69 años. Tiene un 100% de NA, por lo 
#que la ELIMINAMOS

### sexnumpartyear ->
#Esta variable refleja el número de parejas del sexo opuesto con las que el
#participante ha tenido algún tipo de relación sexual en los últimos 12 meses y 
#ha sido recogida para participantes de entre 18 y 69 años. Tiene un 100% de NA,
...
#En este dataset tenemos participantes de 18 o mas años
nhanesoldraw %>% skim()
#CATEGORICAS
...
#sido recogida para participantes de 20 años o mas, por lo que imputaremos este 
...
#diabeticos, a pesar de que su número de observaciones no es tan grande como el
#resto de categorías se mantiene. Veamos como queda la distribución
...
#Teniendo en cuenta la cantidad de observaciones de cada categoría y las 
...
#estados de soltería y la tercera categoría representa participantes de 18 o 19
#años, por lo que es probable que la mayoría de participantes se correspondan 
#con un estado de soltería antes que estar viudos, casados o habiendo finalizado
#un matrimonio.
...
### work ->
#Refleja la relación con respecto al trabajo. Tiene menos de 1% de NA.
...
#Teniendo en cuenta los porcentajes de observaciones en cada categoría 
#decidimos reagrupar las categorías Looking y NotWorking
#Los NA se imputan

### healthgen ->
#Refleja una calificación categórica del estado de salud autopercibido y cuenta 
#con 5 categorías. Tiene 15% de NA. Esta variable ha sido preguntada a todos los
...
#Por otro lado, debido a que desconocemos si hay algún tipo de información 
#detrás de no haber contestado a esta variable, a pesar de que pueda tratarse de
#un simple error a la hora de recoger la informacion, teniendo en cuenta la 
#relación de los NA con la variable objetivo y la cantidad de estos, decidimos 
#imputarlos como una nueva categoría "NA"

### littleinterest ->
#Refleja una calificación categórica del número de días que el participante 
#ha tenido interés en hacer cosas. Tiene un 15% de NA. Esta variable ha sido 
...
#Debido a que desconocemos si hay algún tipo de información 
#detrás de no haber contestado a esta variable, a pesar de que pueda tratarse de
#un simple error a la hora de recoger la informacion, teniendo en cuenta la 
#relación de los NA con la variable objetivo y la cantidad de estos, decidimos 
#imputarlos como una nueva categoría "NA"
...
#relación de estos con la variable objetivo decidimos imputar los NA como
#una nueva categoría "NA"
...
### alcohol12plusyr ->
#Se trata de una variable binaria en la que se expone si el participante ha 
#bebido al menos 12 copas en cualquier año. Tiene 17% de NA
...
#participantes de 20 años o más y ademas han respondido Yes a la variable 
...
#recogida para participantes de 20 años o más, por lo que crearemos una 
...
#observaciones, con el objetivo de no distorsionar las otras dos categorías,
...
#menos una vez al mes durante un año). Tiene 70% de NA. Ambas han sido recogidas
#para participantes de entre 18 y 59 años. 
...
## harddrugs ->
#Se trata de una variable binaria en la que se expone si el participante ha
#probado cocaína, crack, heroína o metanfetamina. Tiene 30% de NA y ha sido 
#recogida para participantes de entre 18 y 69 años, por lo que crearemos una 
#categoria nueva NotAsked para aquellos participantes que no hayan respondido a 
#esta variable
...
#participantes de entre 18 y 69 años, por lo que crearemos una categoria nueva 
...
### sexorientation ->
#Esta variable refleja la orientación sexual del participante. Tiene 45% de NA
#y ha sido recogida para participantes de entre 18 y 59 años, por lo que 
...
#Existe tan poca variabilidad en las categorías de la variable que la eliminamos.
#El porcentaje de diabéticos en NotAsked aumenta considerablemente pero esto
#ya se contempla en otras variables dado que en esta categoría se recoge a los 
#participantes de 60 o más años. Por este motivo, eliminamos esta variable
...
### length ->
#Esta variable refleja el largo del partipante para participantes de 3 o menos
#años. Tiene un 100% de NA, por lo que la ELIMINAMOS
...
### testosterone ->
#Esta variable refleja el nivel de testosterona total en ng/dL y ha sido 
#recogida únicamente en la encuesta del año 2011-2012. Tiene un 59% de NA.
#Esto se debe a haber sido recogida únicamente en una de las encuestas
...
### daysphyshlthbad ->
#Esta variable refleja el número de días que el participante ha considerado
#que su salud fisica no fue buena en los últimos 30 días. Tiene 15% de NA.
#En el resumen skim se puede ver que p0=p25=p50=0, p75=3 y p100=30. Por ello y
#debido a la información que representa la convertiremos a categórica agrupando 
#los valores en distintas categorías
...
#Por otro lado, al tratarse de una variable subjetiva, viendo la relación de los
#NA con la variable objetivo y teniendo en cuenta que el hecho de no haber 
#respondido puede significar algo, imputaremos los NA como una categoria más 
#("NA")

### daysmenthlthbad ->
#Esta variable refleja el número de días que el participante ha considerado
#que su salud mental no fue buena en los últimos 30 días. Tiene 15% de NA.
#En el resumen skim se puede ver que p0=p25=p50=0, p75=3 y p100=30. Por ello y
#debido a la información que representa la convertiremos a categórica agrupando 
#los valores en distintas categorías
...
### npregnancies ->
#Esta variable refleja el numero de veces que la participante ha estado 
#embarazada. Tiene 66% de NA y ha sido recogida para mujeres de 20 años o mas. 
#Debido a la información que representa la convertiremos a categórica agrupando 
#los valores en distintas categorías. Además, incluiremos una categoría
#especifica para los hombres los cuales serán representados por la categoria
#"NotApply" y las edades de mujeres que no correponden seran representadas por
#"NotAsked". Veamos la relación de los NA con la variable objetivo
...
#imputarlas en la categoría NA

### nbabies ->
#Esta variable refleja el numero de veces que la participante ha tenido bebés.
#Tiene 68% de NA y ha sido recogida para mujeres de 20 años o mas. Debido a la 
#información que representa la convertiremos en categórica agrupando 
#los valores en distintas categorías. Además incluiremos una categoría
#especifica para los hombres los cuales serán representados por la categoria
...
### age1stbaby ->
#Esta variable refleja la edad en la que la participante ha sido madre por 1ª 
#vez y ha sido recogida para mujeres de 20 años o mas. Tiene un 74% de NA. 
#Debido a la información que representa la convertiremos en categórica agrupando 
#los valores en distintas categorías. Además incluiremos una categoría
#especifica para los hombres los cuales serán representados por la categoria
#"No procede" y las edades de mujeres que no correponden seran representadas por
#"NotAsked"
...
### sleephrsnight ->
#Esta variable refleja el número de horas de sueño que el participante tiene por
#la noche en días de trabajo o fin de semana. Tiene menos de un 1% de NA. Se 
#imputan
...
#p100=7. Debido a la cantidad tan baja de NA decidimos mantenerla como numérica
...
### alcoholday ->
#Esta variable refleja el número medio de bebidas consumidas por el participante
#al día en aquellos días que consume bebidas alcohólicas. Tiene un 45% de NA.
#En el resumen skim se puede ver que p0=p25=1, p50=2, p75=4 y p100=82. Por ello 
#y debido a la información que representa la convertiremos a categórica 
#agrupando los valores en distintas categorías
...
### alcoholyear ->
#Esta variable refleja el número de días del año pasado en los que el 
#participante consumió bebidas alcohólicas. Tiene un 29% de NA
...
#Los NA en esta variable no parece a priori que tengan una relación especial con
#el hecho de padecer diabetes. Por ello, se decide mantener esta variable como
#numérica y se imputan los NA

### smokeage -> 
#Esta variable refleja la edad en la que el participante comenzó a fumar de 
#manera regular y ha sido recogida para participantes de 20 o más años. Tiene 
#un 59% de NA. Debido a la informacion que representa convertiremos esta 
#variable a categórica dándole distintas categorías. Además, imputaremos el 
...
#                         breaks=c(-Inf,17,Inf),
#                         labels=c("Antes de los 18","18 o más")),
...
# <chr>           <fct>    <int>  <dbl>
#   1 18 o más        No        2000 0.832 
# 2 18 o más        Yes        403 0.168 
...
#Esta variable refleja la edad a la que el participante comenzó a consumir 
#marihuana de manera regular y ha sido recogida para participantes de entre 18 y
#59 años. Tiene un 85% de NA. Debido a la informacion que representa convertiremos 
#esta variable a categórica dándole distintas categorías. Además, imputaremos el 
#rango de edad no contemplado en una nueva categoria a la que llamaremos 
...
# <chr>           <fct>    <int>  <dbl>
#   1 18 o más        No         712 0.915 
# 2 18 o más        Yes         66 0.0848
...
### sexage ->
#Esta variable refleja la edad en la que el participante tuvo sexo por primera
#vez y ha sido recogida para participantes de entre 18 y 69 años. Tiene un 34% 
#de NA
...
#Los NA en esta variable parece que puedan tener una relación especial con
#el hecho de padecer diabetes. Por ello, se decide pasar esta variable a 
#categórica e imputaremos los NA como una categoria más ("NA")
...
#Esta variable refleja el número de parejas del sexo opuesto con las que el
#participante ha tenido algún tipo de relación sexual en toda su vida y ha sido 
#recogida para participantes de entre 18 y 69 años. Tiene un 31% de NA y debido 
#a la informacion que representa convertiremos esta variable a categórica dándole 
#distintas categorías por rangos de intervalo. Además, imputaremos el rango de 
...
#Esta variable refleja el número de parejas del sexo opuesto con las que el
#participante ha tenido algún tipo de relación sexual en los últimos 12 meses y 
#ha sido recogida para participantes de entre 18 y 69 años. Tiene un 8% de NA y 
#debido a la informacion que representa convertiremos esta variable a categórica
#dándole distintas categorías por rangos de intervalo. Además, imputaremos el 
...
nhanesold <- nhanesoldraw %>%
  ...
  mutate(education= case_when(education=="8th Grade" ~ "8th_Grade",
         ...
         samesex= case_when((age>69) ~ "NotAsked",
                             ...
                             TRUE ~ as.character(samesex)),
  #NUMERICAS
   #habría que ver las siguientes agrupaciones, pero hay que hacer las mismas que en nhanesyoung
...
#saveRDS(nhanesolddep, file = "nhanesolddep.rds")
nhanesolddep <- readRDS("nhanesolddep.rds")
...
#correlación=1 con otras variables. Nos estamos refiriendo a aquellas categorias
#creadas en distintas variables donde se agrupan participantes que no han sido 
#preguntados por ellas. Por ejemplo, sabemos que existe una correlación de 1 
#entre "surveyyr.2009_10" y "tvhrsday.NotAsked" ó "surveyyr.2009_10" y
#"comphrsday.NotAsked" debido a que estas variables, si recordamos, 
#no habían sido recogidas en la encuesta de ese año. También hay correlaciones 
#entre variables que se corresponden a una variable con 2 únicas categorías, por
...
#Dejamos únicamente una de las variables de cada uno de los anteriores grupos,
...
#saveRDS(nhanesold_dummy, file = "nhanesold_dummy.rds")
#nhanesold_dummy <- readRDS("nhanesold_dummy.rds")

#Este dataset lo vamos a usar para árboles y randomforest, 
#tiene k dummies para cada variable categóricas con k categorías 
#(no se ha eliminado ninguna de las k dummies generada por cada variable 
#categorica con k categorias)
#el algoritmo cogerá las mejores variables

#-----7.Preparamos conjuntos train y test y balanceamos-----
#Ahora realizaremos la partición en train-test para que todos los algoritmos
...
#a continuación, aumentamos las observaciones de la clase minoritaria
...
#a continuación, disminuimos las observaciones de la clase mayoritaria
...
#saveRDS(test_nhs, file = "test_nhs.rds")

#-----8.Elimino multicolinealidad entre dummies-----

#Ahora elimino DUMMIES K dejando asi k- dummies para cada variable categórica

#Conjunto de entrenamiento
nhs_mul <- nhs %>% dplyr::select(
...
#Conjunto de test
test_nhs_mul <- test_nhs %>% dplyr::select(
  -race1.Other,-education.NotAsked,-maritalstatus.Widowed,
  -healthgen.NA,
  -daysphyshlthbad.Several,
  -daysmenthlthbad.Several,
...
# saveRDS(nhs_mul, file = "nhs_mul.rds")
# saveRDS(test_nhs_mul, file = "test_nhs_mul.rds")
#-----9. Mas cosas-----
...
#Existen muchas categorías de distintas variables con correlaciones altas debido
