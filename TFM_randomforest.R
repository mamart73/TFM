...
for (n_tree in seq(from=1, to=2000, by=100)){
  ...
  rf<- train(data=nhs,
             ...
             linout = FALSE,ntree=n_tree,nodesize=300,replace=TRUE)
...
ggplot(data=statistics) +
  ...
  labs(title="Número de iteraciones vs. valor sensibilidad",
       x="Número de árboles", y="Sensibilidad") +
...
ggplot(data=statistics) +
  ...
  labs(title="Número de iteraciones vs. valor AUC",
       x="Número de árboles", y="AUC") +
  geom_vline(xintercept = 101, color = '#6F70AA', size=1) +
  theme(text = element_text(family = "lato", size=12), 
        plot.title = element_text(hjust = 0.35),
        plot.subtitle = element_text(hjust=0.35))

#Se estabilizan las medidas de bondad de ajuste sobre las 100 iteraciones
...
for (muestrabag in seq(from=trunc(nrow(nhs)*0.05), to=trunc(nrow(nhs)*0.75), by=1000))
{
  for (minbu in seq(from=round(muestrabag*0.01), to=round(muestrabag*0.15), by=150))
  {
    for (mtrysize in seq(from=10, to=(ncol(nhs)-1), by=25))
    ...
    rf<- train(data=nhs,diabetes ~.,
               ...
               linout = FALSE,sample_size=muestrabag,ntree=100,
...
#muestrabag=713, minbucket=7, mtrysize=110
#muestrabag=1713, minbucket=17, mtrysize=110
#muestrabag=2713, minbucket=27, mtrysize=110

#Ahora hacemos validacion cruzada repetida con los 3 mejores grids de parámetros
...
cruzadarf1<-cruzadarfbin(data=nhs, vardep="diabetes", listconti=c(
  ...
  listclass=c(""),grupos=4,sinicio=1234,repe=10,nodesize=7,replace=TRUE, 
  sampsize=713,ntree=100,mtry=110)
...
cruzadarf2<-cruzadarfbin(data=nhs, vardep="diabetes", listconti=c(
  ...
  'samesex.Yes','bpsys','bpdia','marij.HasTried','marij.MarijUser','marij.NA',
  'marij.Never','marij.NotAsked'),
  listclass=c(""),grupos=4,sinicio=1234,repe=10,nodesize=17,replace=TRUE, 
  sampsize=1713,ntree=100,mtry=110)
...
cruzadarf3<-cruzadarfbin(data=nhs, vardep="diabetes", listconti=c(
  ...
  'samesex.Yes','bpsys','bpdia','marij.HasTried','marij.MarijUser','marij.NA',
  'marij.Never','marij.NotAsked'),
  listclass=c(""),grupos=4,sinicio=1234,repe=10,nodesize=27,replace=TRUE, 
  sampsize=2713,ntree=100,mtry=110)

cruzadarf1$modelo="rf_s713_n7"
cruzadarf2$modelo="rf_s1713_n17"
cruzadarf3$modelo="rf_s2713_n27"
...
#El mejor modelo de random forest es cruzadarf3$modelo="rf_s2713_n27"
#-----6. Variables importantes random forest-----
library(randomForest)
set.seed(1234)
imp_best_rf <- randomForest(diabetes~., 
                          data=nhs, mtry=110, ntree=100, sampsize=2713, 
                          nodesize=27, replace=TRUE)
...
#age, totchol, weight, healthgen.Fair y height
